<x-admin-layout>
    <div class="px-4 py-6 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-[1300px] space-y-6">
            <x-layout.page-header title="Payment verification events" description="Server-owned Stripe webhook processing, failure, retry, and completion audit trail.">
                <x-slot:actions>
            ...
            <form method="GET" action="{{ route('admin.payment-events.index') }}"
                class="grid gap-2 rounded-lg border border-[rgb(var(--cp-line))] bg-[rgb(var(--cp-surface))] p-3 shadow-sm sm:grid-cols-[180px_1fr_auto]">
                <label class="sr-only" for="status">Status</label>
                <select id="status" name="status"
                    class="rounded-lg border-[rgb(var(--cp-line))] bg-stone-100 text-sm font-semibold text-slate-800 shadow-sm focus:border-emerald-600 focus:ring-emerald-600">
                ...
                <input id="session_id" name="session_id" type="text" value="{{ request('session_id') }}" placeholder="Search session id"
                    class="min-w-0 rounded-lg border-[rgb(var(--cp-line))] bg-stone-100 text-sm font-semibold text-slate-800 shadow-sm focus:border-emerald-600 focus:ring-emerald-600">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white text-sm">
                        @forelse($paymentEvents as $event)
                            <tr class="transition hover:bg-slate-50">
                                <td class="px-4 py-3">
                                    <a href="{{ route('admin.payment-events.show', $event) }}" class="font-semibold text-emerald-700 hover:text-emerald-900">
                                ...
                                <td class="cp-tabular px-4 py-3 font-bold text-[rgb(var(--cp-ink))]">
